...
%      function named CALLBACK in TIAOZHIJIETIAO.M with the given input arguments.
%
...
% Last Modified by GUIDE v2.5 24-May-2019 20:17:14
...
switch obj
    case 'AM相干'
        ...
        global AMxgcj
        axes(handles.axes1);
        plot(AMxgcj.time,AMxgcj.signals(1).values*gain);
        axes(handles.axes2);
        plot(AMxgcj.time,AMxgcj.signals(2).values*gain);
    case 'AM包络'
        open('AMbaoluo');sim('AMbaoluo');
        set(handles.edit5,'String','带通滤波后的已调信号');
        global AMbltz
        axes(handles.axes1);
        plot(AMbltz.time,AMbltz.signals(1).values*gain);
        axes(handles.axes2);
        plot(AMbltz.time,AMbltz.signals(2).values*gain);
    case 'DSB'
        open('DSB');sim('DSB');
           set(handles.edit5,'String','已调信号乘载波信号');
        global DSBtiaozhi
        axes(handles.axes1);
        plot(DSBtiaozhi.time,DSBtiaozhi.signals(1).values*gain);
         axes(handles.axes2);
        plot(DSBtiaozhi.time,DSBtiaozhi.signals(2).values*gain);
    case 'USB滤波'
        open('USBlvbo');sim('USBlvbo');
           set(handles.edit5,'String','已调信号乘载波信号');
        global USBlbtz
        axes(handles.axes1);
        plot(USBlbtz.time,USBlbtz.signals(1).values*gain);
         axes(handles.axes2);
        plot(USBlbtz.time,USBlbtz.signals(2).values*gain);
    ...
    case 'USB相移'
        ...
        global USB23
        axes(handles.axes1);
        plot(USB23.time,USB23.signals(1).values*gain);
         axes(handles.axes2);
        plot(USB23.time,USB23.signals(2).values*gain);
    ...
    case 'LSB滤波'
        ...
        global LSBlbtz
        axes(handles.axes1);
        plot(LSBlbtz.time,LSBlbtz.signals(1).values*gain);
         axes(handles.axes2);
        plot(LSBlbtz.time,LSBlbtz.signals(2).values*gain);
        ...
        global LSB3
        axes(handles.axes1);
        plot(LSB3.time,LSB3.signals(1).values*gain);
         axes(handles.axes2);
        plot(LSB3.time,LSB3.signals(3).values*gain);
...
switch obj
    ...
    case 'AM相干'
        ...
        global AMxgjt
        axes(handles.axes3);
        plot(AMxgjt.time,AMxgjt.signals(1).values*gain);
        axes(handles.axes4);
        plot(AMxgjt.time,AMxgjt.signals(2).values*gain);
   
    case 'AM包络'
         open('AMbaoluo');sim('AMbaoluo');
         global AMbljt
         axes(handles.axes3);
         plot(AMbljt.time,AMbljt.signals(1).values*gain);
         axes(handles.axes4);
         plot(AMbljt.time,AMbljt.signals(3).values*gain);
    
    case 'DSB'
        open('DSB');sim('DSB');
        global DSBjietiao
         axes(handles.axes3);
        plot(DSBjietiao.time,DSBjietiao.signals(1).values*gain);
         axes(handles.axes4);
        plot(DSBjietiao.time,DSBjietiao.signals(2).values*gain);
   
    case 'USB滤波'
        open('USBlvbo');sim('USBlvbo');
        global USBlbjt
        axes(handles.axes3);
        plot(USBlbjt.time,USBlbjt.signals(1).values*gain);
         axes(handles.axes4);
        plot(USBlbjt.time,USBlbjt.signals(2).values*gain);
    ...
    case 'USB相移'
        ...
        global USB3
        axes(handles.axes3);
        plot(USB2.time,USB2.signals(2).values*gain);
         axes(handles.axes4);
        plot(USB3.time,USB3.signals(2).values*gain);
   
    case 'LSB滤波'
         open('LSBlvbo');sim('LSBlvbo');
        global LSBlbjt
        axes(handles.axes3);
        plot(LSBlbjt.time,LSBlbjt.signals(1).values*gain);
         axes(handles.axes4);
        plot(LSBlbjt.time,LSBlbjt.signals(2).values*gain);
    ...
    case 'LSB相移'
        ...
        global LSB4
        axes(handles.axes3);
        plot(LSB2.time,LSB2.signals(2).values*gain);
         axes(handles.axes4);
        plot(LSB4.time,LSB4.signals(2).values*gain);
...
checkvalue=get(handles.checkbox1,'value');
if checkvalue==0
amplitude=get(handles.slider5,'value');
set(handles.edit11,'String',num2str(amplitude));
end
% --- Executes during object creation, after setting all properties.
...
if check==0
strr=get(handles.edit11,'String');
set(handles.slider5,'value',str2num(strr));
end
% --- Executes during object creation, after setting all properties.
